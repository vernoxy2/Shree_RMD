...
const Card = ({ item }) => (
  <div className="relative rounded-3xl">
    <img src={item.img} alt={item.name} className="w-full object-cover rounded-2xl" />
...
const ServicesSnapshot = () => {
  ...
  const getDeptFromSlug = () => {
    ...
    return DepartmentsList.find((d) => d.id === deptId)          // find by string id
      ?? DepartmentsList[0];                                      // fallback
  };
  ...
  return (
    <div className="bg-Bg">
      <PrimaryHeader
        BgImg={TeamBg}
        HeadLine={<>Our Team / Services <br /> snapshot section</>}
      />
        ...
        <div className="w-8/12 space-y-8">
          ...
          <div>
            <p className="text-xl mb-4">
              <span className="text-primary font-bold">{selected.highlight}</span>
              {" "}- {selected.desc1}
            </p>
          ...
          <div className="grid grid-cols-2 gap-8">
            {selected.object1.map((item) => <Card key={item.id} item={item} />)}
          </div>
          <hr />
          <div className="space-y-6">
            <h1>Our Doctors</h1>
            <div className="grid grid-cols-2 gap-8">
              {selected.object2.map((item) => <Card key={item.id} item={item} />)}
            </div>
          </div>
        </div>
